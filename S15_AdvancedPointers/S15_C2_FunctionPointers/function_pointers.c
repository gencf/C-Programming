...
int (*funcPtrArray[]) (int, int) = { &add,&sub,&mult,&div };

/// Typedef can be used to initialize Array of pointers to functions
// typedef int (*funcPtr)(int, int);
// funcPtr funcPtrArray[] = { &add,&sub,&mult,&div };

// performs the given operation on elements of the arrays
int* performOp(int *a, int *b, int size, int (*funcPtr) (int, int) /* ADD FUNCTION POINTER PARAMETER HERE */);
...
int main( ){
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size, funcPtrArray[j]);		
...
int* performOp(int *a, int *b, int size, int (*funcPtr) (int, int))
{
    // INCLUDE YOUR CODE HERE
	int *arrPtr = malloc(size * sizeof(int));
		
	for(int i = 0; i < size; i++){
		arrPtr[i] = (*funcPtr) (a[i], b[i]);
	}
